...
class NurseListAllView(APIView):
...
class NurseDetailByIdView(APIView):
    def get(self, request, *args, **kwargs):
        nurse_id = kwargs.get('nurse_id')  # Get the nurse ID from the URL
        try:
            nurse = Nurse.objects.get(nurse_id=nurse_id)
        except Nurse.DoesNotExist:
            return Response({"error": "Nurse not found"}, status=status.HTTP_404_NOT_FOUND)
        
        serializer = NurseListSerializer(nurse)
        return Response(serializer.data, status=status.HTTP_200_OK)
...
class NurseUpdateView(APIView):
    def put(self, request, *args, **kwargs):
        ...
        serializer = NurseUpdateSerializer(nurse, data=request.data)
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = NurseUpdateSerializer(nurse, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Nurse updated successfully"}, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class TotalNurseView(APIView):
    def get(self, request, *args, **kwargs):
        total_nurses = Nurse.objects.count()
        if total_nurses == 0:
            return Response({"error": "No nurses found."}, status=status.HTTP_404_NOT_FOUND)
        
        return Response({"total_nurses": total_nurses}, status=status.HTTP_200_OK)
